...
async function governedRunning(ctx, taskStore, dir) {
  ...
  assert.ok((await taskStore.claim(task.id, runId, { lease_seconds: 300 })).claimed);
...
test('half-completed governed completion (Change PREFLIGHT, task running, lease dead) converges task to in_review', async (t) => {
  ...
  // Simulate the crash: task stays running, lease now expired. Read the
  // post-claim record (artifacts of create() were stale versus claim times).
  const claimed = await taskStore.get(task.id);
  const stale = Number(claimed.lease_expires_at ?? 0);
  await ctx.taskChangeControl.reconcileTaskChange(task.id, { simulatedExpiryAt: stale + 1 });
  const after = await taskStore.get(task.id);
